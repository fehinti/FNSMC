...
smc.k = 50;         % sliding surface parameter
...
rnn.eta5 = 0.1;       % E_hat learning rate
rnn.neuron_N = 5;  % number of neurons in hidden layer
...
% Run simulation
sim( 'ABS_contDyn_model_SMC_FZL_NN.slx' );

% Extract simulation time
simTime = noABS.v.Time;

% Vehicle with SMC_ABS (interpolation)
v_SMC_ABS      = interp1( simTime, SMC_ABS.v.Data,      time );
x_SMC_ABS      = interp1( simTime, SMC_ABS.x.Data,      time );
lambda_SMC_ABS = interp1( simTime, SMC_ABS.lambda.Data, time );
Tb_SMC_ABS     = interp1( simTime, SMC_ABS.Tb.Data,     time );
...
% Vehicle with WSC_ABS (interpolation)
v_WSC_ABS      = interp1( simTime, WSC_ABS.v.Data,      time );
x_WSC_ABS      = interp1( simTime, WSC_ABS.x.Data,      time );
lambda_WSC_ABS = interp1( simTime, WSC_ABS.lambda.Data, time );
Tb_WSC_ABS     = interp1( simTime, WSC_ABS.Tb.Data,     time );
...
h_SMC_ABS   = plot( lambda_SMC_ABS(1), yL(2), 'color', ...
               'red',  'LineWidth', 2 );
hnoABS = plot( lambda_noABS(1), yL(2), 'color', ...
               'blue', 'LineWidth', 2 );
h_WSC_ABS   = plot( lambda_WSC_ABS(1), yL(2), 'color', ...
               'green',  'LineWidth', 2 );

legend( [ h_SMC_ABS, hnoABS, h_WSC_ABS ], { 'SMC-ABS', 'noABS', 'WSC-ABS' } );

n = floor(tf/dt);
for i = 1:frame_skip:n
    h_SMC_ABS   = plot( [ lambda_SMC_ABS(i),   lambda_SMC_ABS(i)   ], yL, 'color', ...
    ...
    h_WSC_ABS   = plot( [ lambda_WSC_ABS(i),   lambda_WSC_ABS(i)   ], yL, 'color', ...
                   'green',  'LineWidth', 2, 'HandleVisibility', 'off' );
...
plot( [ lambda_SMC_ABS(i),   lambda_SMC_ABS(i)   ], yL, 'color', ...
      'red',  'LineWidth', 2, 'HandleVisibility', 'off' );
plot( [ lambda_noABS(i), lambda_noABS(i) ], yL, 'color', ...
      'blue', 'LineWidth', 2, 'HandleVisibility', 'off' );
plot( [ lambda_WSC_ABS(i),   lambda_WSC_ABS(i)   ], yL, 'color', ...
      'green',  'LineWidth', 2, 'HandleVisibility', 'off' );
...
h_SMC_ABS   = plot( lambda_SMC_ABS(1),   Tb_SMC_ABS(1),   'color', 'red',  ...
               'Linewidth', 2 );
hnoABS = plot( lambda_noABS(1), Tb_noABS(1), 'color', 'blue', ...
               'Linewidth', 2 );
h_WSC_ABS   = plot( lambda_WSC_ABS(1),   Tb_WSC_ABS(1),   'color', 'green',  ...
               'Linewidth', 2 );

% legend( [ h_SMC_ABS, hnoABS ], { 'ABS', 'noABS' } );
legend( [ h_SMC_ABS, hnoABS, h_WSC_ABS ], { 'SMC-ABS', 'noABS', 'WSC-ABS' } );

n = floor(tf/dt);
for i = 2:frame_skip:n
    h_SMC_ABS   = plot( lambda_SMC_ABS(1:i),   Tb_SMC_ABS(1:i),   'color', 'red',  ...
                   'Linewidth', 2, 'HandleVisibility', 'off' );
    hnoABS = plot( lambda_noABS(1:i), Tb_noABS(1:i), 'color', 'blue', ...
                   'Linewidth', 2, 'HandleVisibility', 'off' );
    h_WSC_ABS   = plot( lambda_WSC_ABS(1:i),   Tb_WSC_ABS(1:i),   'color', 'green',  ...
                   'Linewidth', 2, 'HandleVisibility', 'off' );
    circ_SMC   = plot( lambda_SMC_ABS(i), Tb_SMC_ABS(i), 'X', 'LineWidth', 2, ...
                   'HandleVisibility', 'off' );
    circ_WSC   = plot( lambda_WSC_ABS(i), Tb_WSC_ABS(i), 'O', 'LineWidth', 2, ...
                   'HandleVisibility', 'off' );
    ...
    set( hnoABS, 'Visible', 'off' );
    set( h_WSC_ABS,   'Visible', 'off' );
    set( circ_SMC,   'Visible', 'off' );
    set( circ_WSC,   'Visible', 'off' );
end

plot( lambda_SMC_ABS(1:i),   Tb_SMC_ABS(1:i),   'color', 'red',  ...
      'Linewidth', 2, 'HandleVisibility', 'off' );
plot( lambda_noABS(1:i), Tb_noABS(1:i), 'color', 'blue', ...
      'Linewidth', 2, 'HandleVisibility', 'off' );
plot( lambda_WSC_ABS(1:i),   Tb_WSC_ABS(1:i),   'color', 'green',  ...
      'Linewidth', 2, 'HandleVisibility', 'off' );
...
for i = 1:length( brake_times )
    x_brake = x_SMC_ABS(time==brake_times(i));
...
for i = 1:length( no_brake_times )
    x_no_brake = x_SMC_ABS(time==no_brake_times(i));
...
h2 = plot( [ x_WSC_ABS(1),   x_WSC_ABS(1)   ], [ 30, 30 ], '--b', 'LineWidth', 2);
...
for i = 2:frame_skip:n
    ...
    %       'LineWidth', 2, 'HandleVisibility', 'off' );
    plot( [ x_WSC_ABS(1),   x_WSC_ABS(i)   ], [ 30, 30 ], '--b', ...
          'LineWidth', 2, 'HandleVisibility', 'off' );
